...
Global $app_name = "Test Automation Gap Analysis"
...
_SQLite_Exec(-1, "CREATE TABLE SubTask (Key,Summary,Description,StoryKey,FixVersion,Status,EstimatedTime,TimeSpent,ProgressPercent);") ; CREATE a Table
...
$issue = _JiraGetSearchResultKeysSummariesAndIssueTypeNames("summary,issuetype", "project = QA AND issuetype = Epic AND labels in (Core) AND labels in (Automation)")
...
$issue = _JiraGetSearchResultKeysSummariesIssueTypeNameEpicKeyRequirements("summary,issuetype,customfield_10008,labels,fixVersions,status", "project = QA AND issuetype = Story AND labels in (Core) AND labels in (Automation)")
...
$issue = _JiraGetSearchResultKeysSummariesIssueTypeNameStoryKeyRequirements("summary,description,issuetype,parent,labels,fixVersions,status,aggregateprogress", "project = QA AND issuetype = Sub-task AND labels in (Core) AND labels in (Automation)")

for $i = 0 to (UBound($issue) - 1) Step 11

	$query = "INSERT INTO SubTask (Key,Summary,Description,StoryKey,FixVersion,Status,EstimatedTime,TimeSpent,ProgressPercent) VALUES ('" & $issue[$i] & "','" & $issue[$i + 1] & "','" & $issue[$i + 2] & "','" & $issue[$i + 4] & "','" & $issue[$i + 6] & "','" & $issue[$i + 7] & "','" & $issue[$i + 8] & "','" & $issue[$i + 9] & "','" & $issue[$i + 10] & "');"
	_FileWriteLog($log_filepath, "Story " & ($i + 1) & " of " & UBound($issue) & " = " & $query)
	_SQLite_Exec(-1, $query) ; INSERT Data
Next
...
; select Epic.Summary as "Sub Category", Story.Summary as "Pages", SubTask.Summary as "Processes", case when SubTask.Status = 'Waiting For Build' then 'Yes' when SubTask.Status = 'In Progress' or SubTask.Status = 'Beta Testing' or SubTask.Status = 'Ready' or SubTask.Status = 'Closed' or SubTask.Status = 'Cancelled' or SubTask.Status = 'Resolved' or SubTask.Status = 'Done' or SubTask.Status = 'Test Run' then 'Yes' else 'No' end as "Coverage", '' as "Percentage Completed", '' as "Platform Coverage", '' as "Notes" from Epic left join Story on Epic.Key = Story.EpicKey left join SubTask on Story.Key = SubTask.StoryKey
...
Update_Confluence_Page("https://janisoncls.atlassian.net", $jira_username, $jira_password, "JAST", "390496535", "390922241", "Test Automation Gap Analysis", $html)
...
Func Create_HTML_Report($confluence_html = False)
	...
	if $confluence_html = True Then
		...
		SQLite_to_HTML_table("select '<a href=""https://janisoncls.atlassian.net/browse/' || Epic.Key || '"" target=""_blank"">' || Epic.Summary || '</a>' as ""Sub Category"", '<a href=""https://janisoncls.atlassian.net/browse/' || Story.Key || '"" target=""_blank"">' || Story.Summary || '</a>' as ""Pages"", '<a href=""https://janisoncls.atlassian.net/browse/' || SubTask.Key || '"" target=""_blank"">' || SubTask.Summary || '</a>' as ""Processes"", case when SubTask.Status = 'Waiting For Build' then 'Yes' when SubTask.Status = 'In Progress' or SubTask.Status = 'Beta Testing' or SubTask.Status = 'Ready' or SubTask.Status = 'Closed' or SubTask.Status = 'Cancelled' or SubTask.Status = 'Resolved' or SubTask.Status = 'Done' or SubTask.Status = 'Test Run' then 'Yes' else 'No' end as ""Coverage"", case when length(SubTask.ProgressPercent) > 0 then SubTask.ProgressPercent || '%' else '' end as ""Percentage Completed"", '' as ""Platform Coverage"", SubTask.Description as ""Notes"" from Epic left join Story on Epic.Key = Story.EpicKey left join SubTask on Story.Key = SubTask.StoryKey order by Epic.Summary;", "150,200,200,50,60,100,500", "ts,tc,tc,ts,tc,tc,tc", "", "", "1,1,0,0,0,0,0", $confluence_html)
	EndIf

	$html = $html & 		"<br /><br /><a href=\""https://github.com/seanhaydongriffin/Test-Automation-Gap-Analysis/releases/download/v0.1/Test.Automation.Gap.Analysis.portable.exe\"">Click to update this page</a>" & @CRLF
